      ...
      <div className="glass-panel rounded-3xl overflow-hidden border border-white/5 shadow-2xl">
        <table className="w-full text-left text-xs text-slate-300">
          ...
          <tbody className="divide-y divide-white/10">
            {loading ? (
              <tr><td colSpan="5" className="text-center py-12 text-slate-400 animate-pulse font-semibold">Loading users...</td></tr>
            ) : users.length === 0 ? (
              <tr><td colSpan="5" className="text-center py-12 text-slate-500 font-semibold">No users found.</td></tr>
            ) : (
              users.map(user => {
                const isBlocked = user.authBlock && (user.authBlock.isBlocked === true || !!user.authBlock.blockedUntil);
                return (
                  <tr key={user._id} className="hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4 font-mono text-slate-500 text-[11px] truncate max-w-[100px]">{user._id}</td>
