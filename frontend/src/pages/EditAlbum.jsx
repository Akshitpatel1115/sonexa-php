...
import { updateAlbum, getAlbumById, getAllMusic, deleteMusic, getAllAlbums } from "../services/musicService";
...
const EditAlbum = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [allMusics, album] = await Promise.all([
          getAllMusic(),
          getAlbumById(id)
        ]);
        ...
        const currentId = user?._id ? user._id.toString() : user?.id?.toString();
      ...
      <div className="relative w-full rounded-3xl p-6 sm:p-8 glass-panel border border-white/5 overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl bg-gradient-to-r from-purple-900/40 via-indigo-950/60 to-slate-900/80">
        <div className="flex items-center gap-5">
          ...
          <div className="flex flex-col">
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight mt-1">Edit Album</h1>
            <p className="text-xs sm:text-sm text-slate-300 font-medium mt-1">Update your album's title and modify its spatial tracklist.</p>
          </div>
        </div>
      </div>
